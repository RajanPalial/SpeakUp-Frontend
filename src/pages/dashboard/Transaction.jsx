...
/* ─── status config ─── */
const STATUS = {
  0: { label: "Pending", color: C.amber, bg: C.amberSoft, dot: "🟡" },
  1: { label: "In Progress", color: C.blue, bg: C.blueSoft, dot: "🔵" },
  2: { label: "Success", color: C.green, bg: C.greenSoft, dot: "🟢" },
  3: { label: "Failed", color: C.red, bg: C.redSoft, dot: "🔴" },
};

const StatusBadge = ({ status }) => {
  const s = STATUS[status] ?? STATUS[3];
  return (
    <span style={{
      display: "inline-flex",
      alignItems: "center",
...
const Truncate = ({ value, chars = 8 }) => {
  if (!value) return <span style={{ color: C.muted }}>—</span>;
  const short = value.length > chars * 2 + 3
...
/* ─── Custom scrollable parent-department dropdown ─── */
const ParentDepartmentDropdown = ({ departments, dropdownLoading, value, onChange }) => {
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {dropdownLoading ? "Loading departments..." : (selected ? selected.name : "Select Parent Department")}
        </span>
        <span style={{
          marginLeft: 8,
          fontSize: 10,
      ...
      {open && !dropdownLoading && (
        <>
          ...
          <div style={{
            ...
            borderRadius: 8,
            zIndex: 2,
            maxHeight: 220,          // ← fixed max height
            overflowY: "auto",       // ← scrollbar when items overflow
...
const Transactions = () => {
  ...
  const addDepartment = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        "/department/add-department",
        {
          ...
          parent_department_id: department.parent_department_id
            ? Number(department.parent_department_id)
            : null,
        },
        {
          headers: {
      ...
      if (res.data.status === 1) {
        ...
        // Refresh department list if you have a listing API
        // getDepartments();
      } else {
  ...
  const getDepartmentDropdown = async () => {
    try {
      ...
      console.log("Department Dropdown:", res.data);
  ...
  const getDepartments = async () => {
    try {
      ...
      const res = await axios.get("/department/get-department", {
        params: {
          searchParam: state.search || null,
  ...
  const deleteDepartment = async (id) => {
    try {
      ...
      const res = await axios.delete(
        `/department/delete-department?id=${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <div>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                {(state.search || state.status) && (
                  <button
                    onClick={() => setState({ ...state, search: "", status: "", currentPage: 1 })}
                    style={{
                ...
                <table style={{
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      rows.rows.map((dept, index) => (
                        <tr key={dept.id} className="txn-row">
                          ...
                          <td style={tdStyle}>{dept.member}</td>
                            ...
                            >
                              {dept.is_root ? (
                                ...
                                <>
                                  ...
                                  {/* Delete */}
                                  <button
                                    onClick={() => { setOpenModal(true); setDeptId(dept.id) }}
                                    style={{
                                      display: "inline-flex",
                                      alignItems: "center",
                                      gap: "6px",
          ...
          >
            ...
            >
              ...
              <ParentDepartmentDropdown
                ...
                onChange={(val) => setDepartment({ ...department, parent_department_id: val })}
              />
            </div>
...

      {
        openModal && (
          <div
            onClick={() => { setOpenModal(false); setDeptId(""); }}
            ...
            >
              ...
              <div style={{ padding: "18px 22px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <h5 style={{ margin: 0, color: C.text, fontSize: 16, fontWeight: 700 }}>Delete Department</h5>
                </div>
                <button
                  onClick={() => { setOpenModal(false); setDeptId(""); }}
              ...
              <div style={{ padding: "14px 22px", borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "flex-end", gap: 10 }}>
                ...
                >Cancel</button>
                <button
                  onClick={() => { deleteDepartment(deptId); setOpenModal(false); setDeptId(""); }}
                  disabled={loading}
